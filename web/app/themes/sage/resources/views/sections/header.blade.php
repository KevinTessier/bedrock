  ...
  <div class="content-grid py-3">
    ...
    @if (has_nav_menu('primary_navigation'))
      ...
      <button type="button"
        ...
        aria-expanded="false" aria-controls="primary-menu" data-nav-toggle
        data-label-open="{{ __('Open main menu', 'sage') }}" data-label-close="{{ __('Close main menu', 'sage') }}">
        <span class="sr-only" data-nav-toggle-label>{{ __('Open main menu', 'sage') }}</span>
        {{-- Icône ouverte (3 barres) --}}
        <svg class="nav-toggle__icon-open size-6" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        ...
        {{-- Icône fermée (croix) --}}
        <svg class="nav-toggle__icon-close size-6" viewBox="0 0 24 24" fill="none" stroke="currentColor"
          stroke-width="2" stroke-linecap="round" aria-hidden="true" focusable="false">
      ...
      <nav id="primary-menu" class="nav-primary" aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}"
        data-nav-panel data-submenu-label="{{ __('Submenu of %s', 'sage') }}">
        {!! wp_nav_menu([
